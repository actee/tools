/*
	...
	INput now start and finish port
*/
#include <stdio.h>
#include <fcntl.h>
#include <string.h>
...
int main (int argc, char *argv[])
{
	...
	if(argc < 4){printf("Usage: %s <target IP> <start port> <finish port>\n",argv[0]);exit(0);}
	if(atoi(argv[2]) == 0 || atoi(argv[3]) == 0){printf("Please specify valid port numbers\n");exit(0);}
	printf("%d %d\n",atoi(argv[2]),atoi(argv[3]));
	
	// because system calls
	char fn[256];
	sprintf(fn,">> /tmp/ports_%s",argv[1]);
	system(fn);
	sprintf(fn,"ports=/tmp/ports_%s",argv[1]);
	putenv(fn);
	printf("File with ports on /tmp/ports_%s or $ports\n",argv[1]);
	...
	printf("Sent packets to all ports\n");
